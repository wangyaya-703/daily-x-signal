...
如果系统里没有 `python3.11`，但仓库内 `.venv/bin/python` 可用，或 `./scripts/run_cli.sh --help` 能正常执行，这只能算“非阻断提示”，不要把它描述成当前不可运行；应明确说“系统级 python3.11 缺失，但 repo 内 runtime 已就绪”。
...
4. 在 following 同步完成后，先用自然语言总结：
   ...
   - 每个方向为什么被推荐
   - 默认会直接写入的关键词
   - 如果后面要用户选择编号，必须在同一条消息里先列出完整编号映射，例如：
     - `1. AI 编程 / Coding Agent / 工作流`
     - `2. Agent 框架 / 多智能体 / 编排`
     - `3. 新模型 / 新功能 / 产品发布`
     - `4. 模型研究 / 论文 / Benchmark`
   - 不要只说“关注方向编号 [1,2,3]（默认推荐）”，却不展示编号对应的方向、推荐原因和关键词
5. 如果是老用户，优先走“快速更新”：
...
如果用户明确说自己是在 OpenClaw 里配置，默认把宿主模式理解为 `openclaw`，不要再把 GitHub fallback 当作主推荐。
