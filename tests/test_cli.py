...
class CliTests(unittest.TestCase):
    ...
    def test_collect_candidate_posts_expands_to_next_batch_when_first_batch_empty(self) -> None:
        authors = [Author(handle=f"a{i}") for i in range(1, 26)]
        config = {
            "x": {
        ...
        def fake_collect_posts(_client, batch, _config, _window):
            ...
            if handles == [f"a{i}" for i in range(1, 21)]:
                return []
            if handles == [f"a{i}" for i in range(21, 26)]:
                return [sample_post]
            return []
        ...
        self.assertEqual([author.handle for author in scanned_authors], [f"a{i}" for i in range(1, 26)])
        self.assertEqual(author_scan_limit, 25)
